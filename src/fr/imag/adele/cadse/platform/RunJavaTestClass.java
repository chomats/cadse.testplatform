...
//		jvm 			= getTestValue("jvm", testName, System.getProperty("java.home").replace ("\\","/")+"/bin/java");
		...
		processBuilder.command(execEclipse, "-application", application, 
				...
				"-Dorg.eclipse.swtbot.screenshots.dir="+screenshots,
		...
		if (debugPort != 0) {
			...
			processBuilder.command().add("-Xrunjdwp:transport=dt_socket,address="+debugPort+",server=y,suspend="+suspendString);
		}
